...
global bg_zoom, half_dimension, bg_height, startx, starty, level_won
level_won=False
bg_zoom=[0.5, 0.5]
...
class Column():
    ...
    def draw(self):
        ...
        #x=int((self.pos[0]-centre[0]-startx+scroll[0][0])*bg_zoom[0])+centre[0]
        x=int((self.pos[0]-centre[0]+scroll[0][0])*bg_zoom[0])+centre[0]
        nextx=int((self.pos[0]+1-centre[0]+scroll[0][0])*bg_zoom[0])+centre[0]
        #y=int((self.pos[1]-centre[1]+starty+scroll[0][1])*bg_zoom[1])+centre[1]
...
while True:
  ...
  for i in range(1, len(objects)):
    ...
    if x_range!="none" and y_range!="none":
      objects[i].draw()
  write(str(len(background_houses)), 200, 200, (200, 50, 178))
  objects[0].draw()
  write(str(objects[0].inventory), 400, 200, (200, 50, 178))
  ...
  write(str(lives), 300, 200, (200, 50, 178))
  pygame.display.update()
  time.sleep(1/fps)
